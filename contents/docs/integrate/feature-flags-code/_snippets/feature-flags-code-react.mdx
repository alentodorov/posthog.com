...
| Hook | Description |
| --- | --- |
| `useFeatureFlagEnabled` | Returns `true`, `false`, or `undefined` (while loading) indicating whether the feature flag is enabled. Pass an optional second argument `defaultValue` to return that value instead of `undefined` while flags are loading. This sends a `$feature_flag_called` event. |
...
> **Tip:** `useFeatureFlagEnabled` returns `undefined` while flags are loading. To avoid handling this third state, pass a default value as the second argument:
>
> ```react
> // Returns false instead of undefined while loading
